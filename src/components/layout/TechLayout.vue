  ...
  <div class="min-h-screen bg-muji-white flex">
    ...
    <aside class="fixed left-0 top-0 h-full w-60
      ...
      <nav class="flex-1 px-3 py-4 overflow-y-auto">
        <div class="px-3 mb-2">
          <span class="text-[10px] font-medium tracking-[0.12em] uppercase text-muji-linen">檢驗作業</span>
        </div>
        <router-link
    ...
    <div class="ml-60 flex-1 flex flex-col min-h-screen">
      <header class="h-14 bg-white border-b border-muji-border flex items-center px-6 gap-4 shadow-xs sticky top-0 z-20">
        <div class="flex-1">
          <h1 class="text-lg font-light text-muji-text tracking-tight">{{ (route.meta.title as string) || '檢驗作業' }}</h1>
        </div>

...
function getStatusLabel(status: string): string {
  const map: Record<string, string> = {
    draft: '待審核', reviewed: '已審核', final: '已核發',
  }
...
<template>
  <div class="p-8 space-y-6">
    <div>
      <h1 class="text-2xl font-light text-muji-text tracking-tight">待核發報告</h1>
      <p class="text-sm text-muji-text-light mt-1">查看等待審核或已核發的報告狀態，高效管理。</p>
    </div>
    ...
    <div class="bg-white rounded-sm border border-muji-border shadow-sm overflow-hidden">
      ...
      <table v-else-if="reportStore.filteredReports.length > 0" class="w-full text-sm">
        <thead>
          <tr class="bg-muji-white border-b border-muji-border">
            ...
            <th class="px-6 py-3 text-left text-2xs font-medium tracking-widest uppercase text-muji-text-light">建立日期</th>
        ...
        <tbody class="divide-y divide-muji-border">
          <tr
            ...
            @click="router.push({ name: 'ReviewDetail', params: { id: report.id } })"
          >
